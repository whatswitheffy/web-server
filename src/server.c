...
char *errorBody = "<html><h1>Error 404<h1><html>\r\n"; 
char *errorHeader = "HTTP/1.1 404\r\n\r\n";
char *strForHTTP = "HTTP/1.1";
char *correctRequest = "HTTP/1.1 200\r\n";
char *contType = "Сontent-type: ";
char *contLen = "Сontent-length: ";
...
void sendHeader(int client_socket, char *type, int len) {
    ...
    write(client_socket, correctRequest, strlen(correctRequest) * sizeof(char));
    write(client_socket, contType, strlen(contType) * sizeof(char));   
    write(client_socket, type, strlen(type) * sizeof(char));
    write(client_socket, "\r\n", 2 * sizeof(char));
    write(client_socket, contLen, strlen(contLen) * sizeof(char));
    write(client_socket, str4, strlen(str4) * sizeof(char));
    write(client_socket, "\r\n\r\n", 4 * sizeof(char));
}

void sendError(int client_socket) {
    write(client_socket, errorHeader, strlen(errorHeader) * sizeof(char));
    write(client_socket, errorBody, strlen(errorBody) * sizeof(char));
    close(client_socket); 
}
...
int main(int argc, char** argv) {
    ...
    while (1) {
        for (clientNum = 0; clientNum < MAX_CLIENTS_NUM; clientNum++) {
            if (pids[clientNum] == 0) {
                break;
            }
        }
